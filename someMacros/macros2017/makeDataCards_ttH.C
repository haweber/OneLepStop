...
//histograms for all
map<string, TH1D*> histSig;//check for signal to clean
...
int getSigUncertainty(double &errup, double &errdown, double origyield, TH1D *hup, TH1D *hdown, int bin);
int getSigUncertaintyOneSided(double &err, double origyield, TH1D *h, int bin);
int getSigError(double &err, double origyield, TH1D *h, int bin);
int getSigErrorRel(double &err, double origyield, TH1D* h, int bin);
...
void makeDataCardsOneBinBasis(int bin, TString signaltype, int analysis, bool dropsigcont, bool correlated, int stop, int lsp, int charg, float xval, bool fakedata, bool nosigsyst, bool nobgsyst){
  ...
  //cout << b << endl;
  if(b<0) return;
  int b3 = histSig["SRyield"]->FindBin(b);
...
bool loadSignalFile(TString signaltype, int analysis){
  ...
  if(signaltype==lastsignalname){//if already open, don't reopen
    if(fsig->IsOpen()){
      ...
      filename = inputdir + "signalyields/Histos_"+anname+"_" + signaltype + ".root";
...
bool loadSignalHistos(int analysis){
  ...
  for(map<string,TH1D*>::iterator h=histSig.begin(); h!=histSig.end();++h){
    if(fsig->GetListOfKeys()->Contains(h->first.c_str())) {
      h->second = (TH1D*)fsig->Get(h->first.c_str());
    } else {
...
bool reloadSignal(TString signaltype, int analysis){
  ...
  for(map<string,TH1D*>::iterator h=histSig.begin(); h!=histSig.end();++h) h->second->Delete();
  fsig->Close();
  delete fsig;
  bool l2 = loadSignalFile(signaltype, analysis);
  bool l4 = loadSignalHistos(analysis);
...
int checkbin(int analysis, int bin, double stop, double lsp, bool dropsigcont){
  ...
  for(int x = 1; x<=histSig["SRyield"]->GetNbinsX();++x){
    if(histSig["SRyield"]->GetBinContent(histSig["SRyield"]->FindBin(x))>0){
      signalexists = true;
      break;
    }
  }
