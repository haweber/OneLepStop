...
int ScanChain( TChain* chain, bool fast = true, int nEvents = -1, string skimFilePrefix = "test") {
  ...
  TFile *fw = new TFile("rootfiles/PhotonMETResolution/NuOrNunuPtVsPhotPt_noPhotStats.root","READ");
  //TFile *fw = new TFile("rootfiles/PhotonMETResolution_reminiAOD/NuOrNunuPtVsPhotPt_noPhotStats.root","READ");
  fw->cd();
  ...
  histonames.push_back("PhotMET_23j_counter");             hbins.push_back(10); hlow.push_back(250.); hup.push_back(750);
  histonames.push_back("PhotMET_4j_counter");              hbins.push_back(10); hlow.push_back(250.); hup.push_back(750);
  histonames.push_back("PhotMET_5j_counter");              hbins.push_back(10); hlow.push_back(250.); hup.push_back(750);

  for(int b = 0; b<3; ++b){
    string added = "";
    if(b==1) added = "upweight";
    if(b==2) added = "downweight";
    for(unsigned int i = 0; i<histonames.size(); ++i){
      string samplename = skimFilePrefix;
      string mapname = added + histonames[i] + "_"+samplename;
  ...
  while ( (currentFile = (TFile*)fileIter.Next()) ) {
    ...
    for( unsigned int event = 0; event < nEventsTree; ++event) {
      ...
      int trigID = -1;
      int trigscal = 0;
      if(PhotPt>600&&HLT_CaloJet500_NoJetID()>0)              { trigID = 0; trigscal = HLT_CaloJet500_NoJetID(); }
      else if(PhotPt>270&&HLT_Photon250_NoHE()>0)             { trigID = 1; trigscal = HLT_Photon250_NoHE(); }
      else if(PhotPt>185&&HLT_Photon175()>0)                  { trigID = 2; trigscal = HLT_Photon175(); }
      else if(PhotPt>170&&HLT_Photon165_HE10()>0)             { trigID = 3; trigscal = HLT_Photon165_HE10(); }
      else if(PhotPt>170&&HLT_Photon165_R9Id90_HE10_IsoM()>0) { trigID = 4; trigscal = HLT_Photon165_R9Id90_HE10_IsoM(); }
      else if(PhotPt>125&&HLT_Photon120_R9Id90_HE10_IsoM()>0) { trigID = 5; trigscal = HLT_Photon120_R9Id90_HE10_IsoM(); }
      else if(PhotPt>95 &&HLT_Photon90_R9Id90_HE10_IsoM()>0 ) { trigID = 6; trigscal = HLT_Photon90_R9Id90_HE10_IsoM(); }
      else if(PhotPt>80 &&HLT_Photon75_R9Id90_HE10_IsoM()>0 ) { trigID = 7; trigscal = HLT_Photon75_R9Id90_HE10_IsoM(); }
      else if(PhotPt>55 &&HLT_Photon50_R9Id90_HE10_IsoM()>0 ) { trigID = 8; trigscal = HLT_Photon50_R9Id90_HE10_IsoM(); }
      else if(PhotPt>125 &&HLT_Photon120()>0 )                { trigID = 9; trigscal = HLT_Photon120(); }
	...
	if (is_duplicate(id) ) continue;
      ...
      float weight2l5j = -1;
      float weight1l5j = -1;
      float errweight2lltmod = -1;
      float errweight2lhtmod = -1;
      float errweight1llMlb = -1;
      float errweight1lhMlb = -1;
      float errweight2l = -1;
      float errweight1l = -1;
      float errweight2l5j = -1;
      float errweight1l5j = -1;
	...
	weight1l5j      = hrat_sim_5j_1ltop   ->GetBinContent(hrat_sim_5j_1ltop   ->FindBin(TMath::Min(PhotPt,(float)999.99)));
      ...
      if(is_data()){
	if(ph_ngoodjets()<=3){
	  errweight2lltmod = hrat_data_23j_ltmod_2l->GetBinError(hrat_data_23j_ltmod_2l->FindBin(TMath::Min(PhotPt,(float)999.99)));
	  errweight2lhtmod = hrat_data_23j_htmod_2l->GetBinError(hrat_data_23j_htmod_2l->FindBin(TMath::Min(PhotPt,(float)999.99)));
	  errweight2l      = hrat_data_23j_2l      ->GetBinError(hrat_data_23j_2l      ->FindBin(TMath::Min(PhotPt,(float)999.99)));
	  errweight1llMlb  = hrat_data_23j_lMlb_1ltop->GetBinError(hrat_data_23j_lMlb_1ltop->FindBin(TMath::Min(PhotPt,(float)999.99)));
	  errweight1lhMlb  = hrat_data_23j_hMlb_1ltop->GetBinError(hrat_data_23j_hMlb_1ltop->FindBin(TMath::Min(PhotPt,(float)999.99)));
	  errweight1l      = hrat_data_23j_1ltop     ->GetBinError(hrat_data_23j_1ltop     ->FindBin(TMath::Min(PhotPt,(float)999.99)));
	} else {
	  errweight2lltmod = hrat_data_4j_ltmod_2l->GetBinError(hrat_data_4j_ltmod_2l->FindBin(TMath::Min(PhotPt,(float)999.99)));
	  errweight2lhtmod = hrat_data_4j_htmod_2l->GetBinError(hrat_data_4j_htmod_2l->FindBin(TMath::Min(PhotPt,(float)999.99)));
	  errweight2l      = hrat_data_4j_2l      ->GetBinError(hrat_data_4j_2l      ->FindBin(TMath::Min(PhotPt,(float)999.99)));
	  errweight1llMlb  = hrat_data_4j_lMlb_1ltop->GetBinError(hrat_data_4j_lMlb_1ltop->FindBin(TMath::Min(PhotPt,(float)999.99)));
	  errweight1lhMlb  = hrat_data_4j_hMlb_1ltop->GetBinError(hrat_data_4j_hMlb_1ltop->FindBin(TMath::Min(PhotPt,(float)999.99)));
	  errweight1l      = hrat_data_4j_1ltop     ->GetBinError(hrat_data_4j_1ltop     ->FindBin(TMath::Min(PhotPt,(float)999.99)));
	}
	errweight2l5j      = hrat_data_5j_2l      ->GetBinError(hrat_data_5j_2l      ->FindBin(TMath::Min(PhotPt,(float)999.99)));
	errweight1l5j      = hrat_data_5j_1ltop   ->GetBinError(hrat_data_5j_1ltop   ->FindBin(TMath::Min(PhotPt,(float)999.99)));

      } else {
	if(ph_ngoodjets()<=3){
	  errweight2lltmod = hrat_sim_23j_ltmod_2l->GetBinError(hrat_sim_23j_ltmod_2l->FindBin(TMath::Min(PhotPt,(float)999.99)));
	  errweight2lhtmod = hrat_sim_23j_htmod_2l->GetBinError(hrat_sim_23j_htmod_2l->FindBin(TMath::Min(PhotPt,(float)999.99)));
	  errweight2l      = hrat_sim_23j_2l      ->GetBinError(hrat_sim_23j_2l      ->FindBin(TMath::Min(PhotPt,(float)999.99)));
	  errweight1llMlb  = hrat_sim_23j_lMlb_1ltop->GetBinError(hrat_sim_23j_lMlb_1ltop->FindBin(TMath::Min(PhotPt,(float)999.99)));
	  errweight1lhMlb  = hrat_sim_23j_hMlb_1ltop->GetBinError(hrat_sim_23j_hMlb_1ltop->FindBin(TMath::Min(PhotPt,(float)999.99)));
	  errweight1l      = hrat_sim_23j_1ltop     ->GetBinError(hrat_sim_23j_1ltop     ->FindBin(TMath::Min(PhotPt,(float)999.99)));
	} else {
	  errweight2lltmod = hrat_sim_4j_ltmod_2l->GetBinError(hrat_sim_4j_ltmod_2l->FindBin(TMath::Min(PhotPt,(float)999.99)));
	  errweight2lhtmod = hrat_sim_4j_htmod_2l->GetBinError(hrat_sim_4j_htmod_2l->FindBin(TMath::Min(PhotPt,(float)999.99)));
	  errweight2l      = hrat_sim_4j_2l      ->GetBinError(hrat_sim_4j_2l      ->FindBin(TMath::Min(PhotPt,(float)999.99)));
	  errweight1llMlb  = hrat_sim_4j_lMlb_1ltop->GetBinError(hrat_sim_4j_lMlb_1ltop->FindBin(TMath::Min(PhotPt,(float)999.99)));
	  errweight1lhMlb  = hrat_sim_4j_hMlb_1ltop->GetBinError(hrat_sim_4j_hMlb_1ltop->FindBin(TMath::Min(PhotPt,(float)999.99)));
	  errweight1l      = hrat_sim_4j_1ltop     ->GetBinError(hrat_sim_4j_1ltop     ->FindBin(TMath::Min(PhotPt,(float)999.99)));
	}
 	errweight2l5j      = hrat_sim_5j_2l      ->GetBinError(hrat_sim_5j_2l      ->FindBin(TMath::Min(PhotPt,(float)999.99)));
	errweight1l5j      = hrat_sim_5j_1ltop   ->GetBinError(hrat_sim_5j_1ltop   ->FindBin(TMath::Min(PhotPt,(float)999.99)));
      }

      if(nEventsTotal%10000==0) cout << weight1l << " +/- " << errweight1l/weight1l*100. << ", " << weight2l << " +/- " << errweight2l/weight2l*100. << endl;

      if(is_data()&&pfmet()>550&&(weight>5||weight*weight1l>5)){
	cout << "NJ " << ph_ngoodjets() << " phMET " << ph_met() << " realMET " << pfmet() << " PhotPt " << PhotPt << " " << weight << " " << weight1l << " " << trigID << " (" << run() << ":" << ls() << ":" << evt() << ")" << endl;
	continue;
      }
      
      if(ph_ngoodjets()<=3){
	histos["PhotMET_23j_counter_"   +samplename]->Fill(ph_met(),1);
	...
	histos["PhotMET_4j_counter_"   +samplename]->Fill(ph_met(),1);
	...
	if(ph_ngoodjets()>=5){
	  histos["PhotMET_5j_counter_"   +samplename]->Fill(ph_met(),1);
      ...
      if(ph_ngoodjets()<=3){
	histos["upweightPhotMET_23j_counter_"   +samplename]->Fill(ph_met(),1);
	histos["upweightPhotMET_23j_noweight_"  +samplename]->Fill(ph_met(),weight);
	histos["upweightPhotMET_23j_1ltop_"     +samplename]->Fill(ph_met(),weight*(weight1l+errweight1l));
	histos["upweightPhotMET_23j_lMlb_1ltop_"+samplename]->Fill(ph_met(),weight*(weight1llMlb+errweight1llMlb));
	histos["upweightPhotMET_23j_hMlb_1ltop_"+samplename]->Fill(ph_met(),weight*(weight1lhMlb+errweight1lhMlb));
	histos["upweightPhotMET_23j_2l_"        +samplename]->Fill(ph_met(),weight*(weight2l+errweight2l));
	histos["upweightPhotMET_23j_ltmod_2l_"  +samplename]->Fill(ph_met(),weight*(weight2lltmod+errweight2lltmod));
	histos["upweightPhotMET_23j_htmod_2l_"  +samplename]->Fill(ph_met(),weight*(weight2lhtmod+errweight2lhtmod));
      }
      else {
	histos["upweightPhotMET_4j_counter_"   +samplename]->Fill(ph_met(),1);
	histos["upweightPhotMET_4j_noweight_"  +samplename]->Fill(ph_met(),weight);
	histos["upweightPhotMET_4j_1ltop_"     +samplename]->Fill(ph_met(),weight*(weight1l+errweight1l));
	histos["upweightPhotMET_4j_lMlb_1ltop_"+samplename]->Fill(ph_met(),weight*(weight1llMlb+errweight1llMlb));
	histos["upweightPhotMET_4j_hMlb_1ltop_"+samplename]->Fill(ph_met(),weight*(weight1lhMlb+errweight1lhMlb));
	histos["upweightPhotMET_4j_2l_"        +samplename]->Fill(ph_met(),weight*(weight2l+errweight2l));
	histos["upweightPhotMET_4j_ltmod_2l_"  +samplename]->Fill(ph_met(),weight*(weight2lltmod+errweight2lltmod));
	histos["upweightPhotMET_4j_htmod_2l_"  +samplename]->Fill(ph_met(),weight*(weight2lhtmod+errweight2lhtmod));
	if(ph_ngoodjets()>=5){
	  histos["upweightPhotMET_5j_counter_"   +samplename]->Fill(ph_met(),1);
	  histos["upweightPhotMET_5j_noweight_"  +samplename]->Fill(ph_met(),weight);
	  histos["upweightPhotMET_5j_1ltop_"     +samplename]->Fill(ph_met(),weight*(weight1l5j+errweight1l5j));
	  histos["upweightPhotMET_5j_2l_"        +samplename]->Fill(ph_met(),weight*(weight2l5j+errweight2l5j));
	}
      }
      if(ph_ngoodjets()<=3){
	histos["downweightPhotMET_23j_counter_"   +samplename]->Fill(ph_met(),1);
	histos["downweightPhotMET_23j_noweight_"  +samplename]->Fill(ph_met(),weight);
	histos["downweightPhotMET_23j_1ltop_"     +samplename]->Fill(ph_met(),weight*(weight1l-errweight1l));
	histos["downweightPhotMET_23j_lMlb_1ltop_"+samplename]->Fill(ph_met(),weight*(weight1llMlb-errweight1llMlb));
	histos["downweightPhotMET_23j_hMlb_1ltop_"+samplename]->Fill(ph_met(),weight*(weight1lhMlb-errweight1lhMlb));
	histos["downweightPhotMET_23j_2l_"        +samplename]->Fill(ph_met(),weight*(weight2l-errweight2l));
	histos["downweightPhotMET_23j_ltmod_2l_"  +samplename]->Fill(ph_met(),weight*(weight2lltmod-errweight2lltmod));
	histos["downweightPhotMET_23j_htmod_2l_"  +samplename]->Fill(ph_met(),weight*(weight2lhtmod-errweight2lhtmod));
      }
      else {
	histos["downweightPhotMET_4j_counter_"   +samplename]->Fill(ph_met(),1);
	histos["downweightPhotMET_4j_noweight_"  +samplename]->Fill(ph_met(),weight);
	histos["downweightPhotMET_4j_1ltop_"     +samplename]->Fill(ph_met(),weight*(weight1l-errweight1l));
	histos["downweightPhotMET_4j_lMlb_1ltop_"+samplename]->Fill(ph_met(),weight*(weight1llMlb-errweight1llMlb));
	histos["downweightPhotMET_4j_hMlb_1ltop_"+samplename]->Fill(ph_met(),weight*(weight1lhMlb-errweight1lhMlb));
	histos["downweightPhotMET_4j_2l_"        +samplename]->Fill(ph_met(),weight*(weight2l-errweight2l));
	histos["downweightPhotMET_4j_ltmod_2l_"  +samplename]->Fill(ph_met(),weight*(weight2lltmod-errweight2lltmod));
	histos["downweightPhotMET_4j_htmod_2l_"  +samplename]->Fill(ph_met(),weight*(weight2lhtmod-errweight2lhtmod));
	if(ph_ngoodjets()>=5){
	  histos["downweightPhotMET_5j_counter_"   +samplename]->Fill(ph_met(),1);
	  histos["downweightPhotMET_5j_noweight_"  +samplename]->Fill(ph_met(),weight);
	  histos["downweightPhotMET_5j_1ltop_"     +samplename]->Fill(ph_met(),weight*(weight1l5j-errweight1l5j));
	  histos["downweightPhotMET_5j_2l_"        +samplename]->Fill(ph_met(),weight*(weight2l5j-errweight2l5j));
	}
      }	
  ...
  for(unsigned int i = 0; i<histonames.size(); ++i){
    ...
    string umapname = "upweight"+histonames[i] + "_"+samplename;
    string dmapname = "downweight"+histonames[i] + "_"+samplename;
    //cout <<  mapname << ":" << endl;
    for(int b = 1; b<= histos[mapname]->GetNbinsX()+1;++b){
      double stderr = histos[mapname]->GetBinError(b);
      double adderr = std::max(fabs(histos[mapname]->GetBinContent(b)-histos[umapname]->GetBinContent(b)),fabs(histos[mapname]->GetBinContent(b)-histos[dmapname]->GetBinContent(b)) );
      histos[mapname]->SetBinError(b,sqrt(pow(stderr,2)+pow(adderr,2)) );
      //cout << "bin " << histos[mapname]->GetXaxis()->GetBinLowEdge(b) << ": " << histos[mapname]->GetBinContent(b) << " +/- " << stderr << " +/- " << adderr << endl;
    }
  }
  ...
  string filename = "rootfiles/PhotonMETResolution/PhotMETDistribution2ndTry_NuorNuNuPtweighted_"+skimFilePrefix+".root";
  //string filename = "rootfiles/PhotonMETResolution_reminiAOD/PhotMETDistribution2ndTry_NuorNuNuPtweighted_"+skimFilePrefix+".root";
  TFile *f = new TFile(filename.c_str(),"RECREATE");
  f->cd();
  for(map<string,TH1F*>::iterator h=    histos.begin(); h!=    histos.end();++h){
    if(h->first.find("upweight") !=string::npos) continue;
    if(h->first.find("downweight") !=string::npos) continue;
    h->second->Write();
  }
  f->Close();
