...
void Combine2DLimitHistos(TString signaltype, TString combine1, TString combine2, float DM2choice, bool prefit, bool fakedata, bool nosigunc, bool nobkgunc, bool dropsigcont, bool correlated){
  ...
  if((int)hExpOrg_t1->GetXaxis()->BinLowEdge(1)!=mStopLow){ cout << "starting bin X different for input1 - exit"; f1->Close(); f2->Close(); exit(0); }
  if((int)hExpOrg_t2->GetXaxis()->BinLowEdge(1)!=mStopLow){ cout << "starting bin X different for input2 - exit"; f1->Close(); f2->Close(); exit(0); }
  if((int)hExpOrg_t1->GetYaxis()->BinLowEdge(1)!=mLSPLow){ cout << "starting bin Y different for input1 - exit"; f1->Close(); f2->Close(); exit(0); }
  if((int)hExpOrg_t2->GetYaxis()->BinLowEdge(1)!=mLSPLow){ cout << "starting bin Y different for input2 - exit"; f1->Close(); f2->Close(); exit(0); }
  if((int)hExpOrg_t1->GetXaxis()->BinWidth(1)!=mStopStep){ cout << "bin width X different for input1 - exit"; f1->Close(); f2->Close(); exit(0); }
  if((int)hExpOrg_t2->GetXaxis()->BinWidth(1)!=mStopStep){ cout << "bin width X different for input2 - exit"; f1->Close(); f2->Close(); exit(0); }
  if((int)hExpOrg_t1->GetYaxis()->BinWidth(1)!=mLSPStep){ cout << "bin width Y different for input1 - exit"; f1->Close(); f2->Close(); exit(0); }
  if((int)hExpOrg_t2->GetYaxis()->BinWidth(1)!=mLSPStep){ cout << "bin width Y different for input2 - exit"; f1->Close(); f2->Close(); exit(0); }
  ...
  for(int x = 1; x<=nbinsx; ++x){
	...
	float stop = hExpOrg->GetXaxis()->GetBinLowCenter(x);//should I use low edge or bin center?
	float lsp = hExpOrg->GetXaxis()->GetBinLowCenter(y);//should I use low edge or bin center?
