...
void MakeCorrelationPlotClaudio(){
  ...
  f->cd();
    ...
    f2->cd();
    TH2D *compressed = (TH2D*)f2->Get("correlationMatrix");
    compressed->SetName("compressed");
    

    // This is the correlation matrix for the standard analysis
    TH2D *correlation = new TH2D("correlationMatrix","",27,0,27,27,0,27);
    TH2D *correlationtext = new TH2D("correlationMatrix","",27,0,27,27,0,27);
    for(int x = 1; x<=27;++x){
        for(int y = 1; y<=27; ++y){
    ...
    for(int x = 1; x<=4;++x){
        for(int y = 1; y<=4; ++y){
	  correlationComp->SetBinContent(x,y,100*compressed->GetBinContent(x,y));
	  correlationtextComp->SetBinContent(x,y,int(round(100*compressed->GetBinContent(x,y))));
